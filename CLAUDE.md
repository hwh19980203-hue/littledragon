...
1. **读取产品信息**：确认产品名称、材质、目标市场和关键特征
2. **提取 Amazon 五点描述**：从参考链接中获取 Bullet Points，分析产品功能描述和风险关键词（如 trademark、patented 等标记）
3. **查看产品图片**：读取产品页中嵌入的图片（产品外观图、细节图），获取视觉信息；通过 Python PIL 分析图片颜色、文字区域等特征
4. **检索相关专利**：使用智慧芽（以图搜图）、USPTO PPUBS、WIPO、CNIPA、EUIPO 等数据库
5. **整理检索结果**：识别相关专利列表
6. **制作权利要求对照表**（Claim Chart）：逐项比对
7. **结合全面覆盖原则和等同原则**评估侵权风险
8. **生成风险评估结论**（cleared / risky / blocked）
9. **更新产品审查页**：写入 Amazon 五点描述、检索记录、图像分析报告、Claim Chart 和分析结论
10. **更新 wiki/index.md 和 wiki/log.md**
